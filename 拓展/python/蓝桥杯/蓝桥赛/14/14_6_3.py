...
def f_min_l(j,l,k):
    ...
    for current in range(l):
        while q and A[current][j]<A[q[-1]][j]:
            q.pop()
        q.append(current)
...
minn=[]
for i in range(n):
    maxx.append(f_max_h(i,len(A[0]),b))
    minn.append(f_min_h(i,len(A[0]),b))
#得到每行长a的，总共m-a+1个的数组
maxx2=[]
minn2=[]
...
for z in range(n): 
    for i in range(m-b+1):
        maxx2.append(f_max_l(z,len(A),a))
        minn2.append(f_min_l(z,len(A),a))
