...
无论哪种线索化，空指针域之可能出现在序列的第一个元素节点和最后一个元素节点
    除非引入虚拟头节点
    中序线索二叉树一定存在两个空指针域

中序线索树的每一个非空线索均指向其祖先节点
    中序序列中一个节点的前驱不可能在它的右子树中，只能在左子树或祖先节点中
        如果一个节点右左子树那么就不会是线索了，所以中序线索树中非空线索一定指向祖先节点

后序线索树中有节点 y ，其有左孩子 x ，y 的右子树高度为 h，那么确定 x 的后继最多需要经过 h 个中间节点
    找 x 的后继也就是找 y 右子树的最左边的叶子
        不算上后继的话就需要走右子树的 h-1 个节点
        注意中间需要经过 y ！
        所以是 h-1+1 = h
*/
